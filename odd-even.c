 ...
 * Run:
 *    mpiexec -n <p> odd-even <g|i> <global_n>
          ...
          int p, MPI_Comm comm)
{
    ...
    if (my_rank % 2 != 0)
    {
        ...
        if (odd_partner == p)
            odd_partner = -1; // Idle during odd phase
    }
    else
    {
        even_partner = my_rank + 1;
        if (even_partner == p)
            even_partner = -1; // Idle during even phase
        odd_partner = my_rank - 1;
    }
